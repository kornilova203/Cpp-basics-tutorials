...
## Функции
Функция - это фрагмент кода, который можно вызвать из другого места программы.

Например, эта фукция принимает целое число, увеличивает его на 1 и возвращает в качестве результата
...
int main() {
    int a = 1;
    cout << increase(a) << endl; // выведет 2
    cout << a; // выведет 1 (переменная a не изменилась!)
    return 0;
}
```

А следующая функция ничего не возвращает, только выводит сообщение:
```c++
void sayHelloTo(string name) {
    cout << "Hello, " << name << "!" << endl; // выводит приветсвие
}
...
Передача массива в функцию и его вывод
```c++
void printArray(int *arr, int size) { // массив - это указатель на первый элемент
...
## Тестирование
Тестирование - это очень важная часть процесса написания программы. Тесты помогают найти ошибки и позволяют убедиться, что программа работает правильно.

Протестируем функцию `increase`. Для этого в main нам нужно написать тесты, используя функцию `assert(значение, которое вернула ф-ция, правильное занчение)`
```c++
#include <assert.h> // в этой библиотеке находится assert
...
```
В данном случае все тесты выполнятся и программа завершится. Если бы функция `increase` работала неправильно, то мы бы увидели ошибку.